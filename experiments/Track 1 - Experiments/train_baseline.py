...
class GCNWithTopK(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        super().__init__()
        self.conv1 = GCNConv(in_channels, hidden_channels)
...
def main():
    ...
    parser.add_argument('--dataset', type=str, choices=['PROTEINS', 'NCI1', 'ENZYMES'], required=True)
    parser.add_argument('--model', type=str, choices=['gcn', 'topk', 'diffpool'], required=True)
    ...
    if args.model == 'gcn':
        model = GCN(num_features, 64, num_classes).to(args.device)
    elif args.model == 'topk':
        model = GCNWithTopK(num_features, 64, num_classes).to(args.device)
    elif args.model == 'diffpool':
        model = GCNWithDiffPool(num_features, 64, num_classes).to(args.device)
    else:
