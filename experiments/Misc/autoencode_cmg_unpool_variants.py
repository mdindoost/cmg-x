...
def run_experiment(dataset_name="Pubmed", out_dir="logs/autoencode", epochs=200, pooling_mode='sum', unpooling_mode='copy'):
    ...
    class CMGAutoencoder(torch.nn.Module):
        ...
        def forward(self, x, edge_index, batch):
            ...
            # Compute additional info for unpooling
            if isinstance(P, list):
                P_cat = torch.cat(P, dim=0)
            else:
                P_cat = P
            cluster_ids = torch.argmax(P_cat, dim=1)
    ...
    save_dir = os.path.join(out_dir, f"{dataset_name}_cmg_{pooling_mode}_unpool_{unpooling_mode}_original_{run_id}")
