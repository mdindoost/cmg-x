...
def analyze_feature_fidelity(X0: torch.Tensor, P: torch.Tensor, reduction='mean'):
    ...
    # Degree per node = sum of assignment row (i.e., cluster size)
...
def convert_to_serializable(obj):
    ...
    elif hasattr(obj, "tolist"):  # NumPy array or Torch tensor
        return obj.tolist()
...
def save_results(result_dict, dataset_name, out_dir="logs/structure_eval/"):
    os.makedirs(out_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    fname = f"{dataset_name}_cmg_eval_{timestamp}.json"
    fpath = os.path.join(out_dir, fname)

    serializable = convert_to_serializable(result_dict)
    with open(fpath, "w") as f:
        json.dump(serializable, f, indent=2)

    print(f"[LOGGED] Saved results to {fpath}")
...
if __name__ == "__main__":
    ...
    result = full_cmg_analysis(data, k=10, d=20, threshold=0.1)
    save_results(result, dataset_name=dataset_name)
